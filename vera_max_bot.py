...
logging.info("MAX_TOKEN: configured" if MAX_TOKEN else "MAX_TOKEN: missing")
logging.info("OPENAI_KEY: configured" if OPENAI_KEY else "OPENAI_KEY: missing")
...
async def ask_claude(question, depth="medium"):
    depths = {
        ...
        "medium": ("Ответь понятно и по существу — 5–7 предложений.", 650),
        "deep": ("Дай вдумчивый ответ, отделяя справку от личного совета.", 1000),
    }
    ...
    try:
        msg = await asyncio.to_thread(
            ...
            max_tokens=max_tok,
            system=(
                "Ты справочный православный помощник, но не священник и не духовник. "
                "Не называй человека чадом, не говори от имени Церкви и не давай благословений. "
                "Не выдумывай цитаты, даты, правила, чудеса и факты о святых. "
                "При сомнении предложи проверить официальный церковный источник. "
                "В вопросах Таинств, поста по здоровью и духовного руководства советуй обратиться к священнику своего прихода. "
                "Отвечай по-русски, спокойно, уважительно и без осуждения. " + system_add
            ),
    ...
    if step == "prayer_for_me_name":
        ...
        try:
            msg = claude_client.messages.create(
                ...
                max_tokens=600,
                system=(
                    "Ты православный помощник. Не представляйся священником. Составь личную молитву для человека "
...
async def get_daily_gospel_max() -> str:
    ...
    try:
        msg = claude_client.messages.create(
            model="claude-sonnet-4-5",
            max_tokens=500,
            system="Ты православный помощник. Не представляйся священником. Дай евангельское чтение дня с коротким толкованием (3-4 предложения). Формат: отрывок из Евангелия (2-3 стиха с указанием источника), потом краткое толкование простым языком. Отвечай по-русски. Без лишних вступлений.",
...
async def get_prayer_of_day_max() -> str:
    ...
    try:
        msg = claude_client.messages.create(
            model="claude-sonnet-4-5",
            max_tokens=600,
            system="Ты православный помощник. Не представляйся священником. Пишешь молитвы тепло и душевно.",
...
async def post_to_channel(text, photo_url=None):
    """Публикует в MAX-канал только текст. photo_url намеренно игнорируется."""
    clean_text = (text or "").strip()
    if not clean_text:
        logging.warning("Канал MAX: пустой текст — пропуск")
        return False
    try:
        result = await max_request("POST", f"messages?chat_id={MAX_CHANNEL_ID}", {"text": clean_text[:4000]})
        if not result:
            raise RuntimeError("MAX API вернул пустой ответ")
        logging.info("Канал MAX: текстовый пост опубликован")
        return True
    except Exception as e:
        logging.error(f"Канал MAX: ошибка публикации: {e}")
        return False
...
MAX_CHANNEL_SENT_FILE = "/root/vera_max_channel_sent.txt"

def _max_channel_sent_keys():
    today = (datetime.utcnow() + timedelta(hours=3)).strftime("%Y-%m-%d")
    try:
        with open(MAX_CHANNEL_SENT_FILE, "r", encoding="utf-8") as fh:
            return {line.strip() for line in fh if line.strip().startswith(today + "_")}
    except FileNotFoundError:
        return set()
    except Exception as e:
        logging.error(f"Канал MAX: не удалось прочитать журнал: {e}")
        return set()

def _mark_max_channel_sent(key: str):
    try:
        with open(MAX_CHANNEL_SENT_FILE, "a", encoding="utf-8") as fh:
            fh.write(key + "\n")
    except Exception as e:
        logging.error(f"Канал MAX: не удалось записать журнал: {e}")

def _channel_midday_prompt(msk_now):
    weekday = msk_now.weekday()
    prompts = {
        0: "Напиши практичный пост: как начать регулярную молитву без перегруза. 4–5 предложений.",
        1: "Напиши спокойный пост о том, как поддержать близкого человека в трудный период. 4–5 предложений.",
        2: "Объясни один православный термин простым языком. Не выдумывай факты. 4–5 предложений.",
        3: "Напиши пост в формате вопрос–ответ о вере для начинающего. 4–5 предложений.",
        4: "Дай бережную памятку о подготовке к исповеди и напомни уточнить детали в своём храме. 4–5 предложений.",
        5: "Расскажи проверяемый общеизвестный эпизод из жизни известного святого и один практический урок. 5–6 предложений.",
        6: "Порекомендуй известную православную книгу или документальный фильм, не выдумывая название и автора. 4–5 предложений.",
    }
    return prompts[weekday]

async def _make_max_channel_text(slot: int, msk_now):
    today_text = f"{msk_now.day:02d}.{msk_now.month:02d}"
    if slot == 8:
        prompt = (
            f"Напиши утренний пост для православного канала на {today_text}: "
            "короткая молитвенная мысль, благодарность и один добрый шаг на день. "
            "4–5 предложений. Начни с 🌅. Без хэштегов и без выдуманных церковных фактов."
        )
        fallback = "🌅 Доброе утро. Начните день с короткой молитвы, благодарности и одного доброго дела. Пусть сегодня будет меньше спешки и больше внимания к людям рядом.\n\n☦️ Православный помощник → @id232007136009_1_bot"
    elif slot == 13:
        prompt = _channel_midday_prompt(msk_now) + " Начни с подходящего эмодзи. Без хэштегов."
        fallback = "🕯️ Вера укрепляется не только словами, но и небольшими поступками: выслушать, помочь, попросить прощения, не ответить злом на зло. Выберите сегодня один такой шаг.\n\n☦️ Православный помощник → @id232007136009_1_bot"
    else:
        prompt = (
            f"Напиши вечерний пост для православного канала на {today_text}: "
            "благодарность за день, спокойное размышление и короткая молитвенная мысль. "
            "4–5 предложений. Начни с 🌙. Без хэштегов."
        )
        fallback = "🌙 День подходит к концу. Вспомните, за что сегодня можно поблагодарить, и спокойно поручите Богу то, что пока не получилось. Завтра будет новый день.\n\n☦️ Православный помощник → @id232007136009_1_bot"
    text = await generate_channel_post(prompt)
    return text or fallback

async def channel_scheduler():
    """Надёжный текстовый автопостинг: 08:00, 13:00 и 20:00 МСК."""
    ...
    while True:
        try:
            ...
            sent = _max_channel_sent_keys()
            due = [h for h in slots if h <= msk_now.hour and f"{today}_{h}" not in sent]
            if due:
                slot = max(due)
                key = f"{today}_{slot}"
                text = await _make_max_channel_text(slot, msk_now)
                ok = await post_to_channel(text)
                if ok:
                    _mark_max_channel_sent(key)
                    if slot == 8:
                        asyncio.create_task(morning_broadcast_max())
                else:
                    logging.error(f"Канал MAX: слот {slot}:00 не опубликован, повтор через 5 минут")
                    await asyncio.sleep(300)
            await asyncio.sleep(30)
        except Exception as e:
            logging.error(f"Канал MAX: ошибка планировщика: {e}")
